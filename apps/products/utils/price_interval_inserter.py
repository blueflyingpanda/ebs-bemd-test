...
class PriceIntervalInserter:
    ...
    def is_valid_date_range(self):
        self.error_message = ''
        if 'end_date' not in self.interval_data:
            return True
    ...
    def get_edge_interval(self, hit_date: date):
        interval = self.queryset.filter(start_date__lt=hit_date, end_date__gt=hit_date)
        if len(interval):
            return interval[0]
        return None

    def shift_edge_intervals(self, left_interval, right_interval):
        if left_interval != right_interval:
            if left_interval:
                left_interval.end_date = self.interval_data['start_date']
                left_interval.save()
            if right_interval:
                right_interval.start_date = self.interval_data['end_date']
                right_interval.save()
        else:
            if left_interval:
                left_interval.end_date = self.interval_data['start_date']
                ...
                new_right_interval = PriceInterval(
                    ...
                    start_date=self.interval_data['end_date'],
    ...
    def _insert(self):
        ...
        ).delete()  # todo check if qs is updated after delete
        self.queryset = self.queryset.all()
        if len(self.queryset):
            self.shift_edge_intervals(
                left_interval=self.get_edge_interval(self.interval_data['start_date']),
                right_interval=self.get_edge_interval(self.interval_data['end_date'])
            )
    ...
    def _convert_null_end_date_to_max_date(self):
        ...
        if not last_interval.end_date:
            ...
            last_interval.save()
        self.queryset = self.queryset.all()
        self.interval_data['end_date'] = self.interval_data.get('end_date', date.max)
